...
- `useCaptura()` do plano A em `frontend/src/captura/CapturaContext.tsx`: `{ enviarFoto, abrirAudio, abrirTexto, ocupado, versao }`. O `CapturaContext` renderiza `<ConfirmacaoRegistro interpretacao aoConfirmar aoDescartar />` — este plano mantém exatamente essas props.
...
- Produces:
  ...
  - Link de alimento: `/registros/<registro.id>/alimentos/<indice>?data=<data>` (a Task 6 cria a rota).
...
Expected: tudo verde. (`FaixaSemanal`, `AnelCalorias`, `BarraMacro` ainda existem sem uso de página; a Task 11 apaga.)
...
- Produces:
  ...
  - `PainelDia({ dia: ProgressoDia; hoje: string })`
...
export default function PainelDia({ dia, hoje }: { dia: ProgressoDia; hoje: string }) {
  const navigate = useNavigate();
  const status = statusVisual(dia.status, dia.data, hoje);
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <Box sx={{ display: 'flex', gap: '9px' }}>
        <BotaoCta
          variante="contorno"
          altura={46}
          onClick={() => navigate(`/relatorio?mes=${dia.data.slice(0, 7)}`)}
...
> Spec pede "abre o menu de captura se for hoje" em `adicionar refeição`. `useCaptura()` não expõe abrir o menu; o botão leva para `/`, onde o `+` está à mão. Se o plano A expuser `abrirMenu()`, trocar a navegação por `abrirMenu()` quando `dia.data === hoje`.
...
`npm run dev`: `/calendario` confere com `04-telas.png` claro/escuro (círculos, hoje em negrito, selecionado preenchido); tocar num dia com registro mostra pílula/kcal e os dois botões; `editar dia` abre `/?data=`; `‹` volta um mês e a URL ganha `?mes=`; `›` desabilita no mês atual; dia futuro mostra "ainda sem registro" sem botão; `/u/<id de amigo>` mostra a grade nova só leitura.
...
### Task 7: Detalhe de item (tela 05) — componente e rota
...
- Produces:
  ...
  - `DetalheAlimento(props: PropsDetalhe)` com
    ...
    ```
    A Task 8 usa o mesmo componente sobre a folha de confirmação.
...
### Task 8: Confirmação da IA (tela 06) como bottom sheet
...
**Interfaces:**
- Consumes: `DetalheAlimento` (Task 7), `corDaRefeicao`, `useRefeicoes`, `somarAlimentos`, `numero`, `milhar`, `BotaoCta`, `RotuloSecao`, `Erro`.
...
### Task 9: Perfil (tela 07) — lista, aparência e seção CONTA
...
- Produces:
  ...
  - Linhas que navegam para `/perfil/metas`, `/perfil/dados`, `/perfil/refeicoes` (Task 10 cria as rotas).
...
Run: `cd frontend && npm test && npm run typecheck && npm run build`
Expected: verde. (O `pages/Perfil.tsx` antigo importava `SecaoMetas`, `SecaoDadosPessoais`, `SecaoRefeicoes`, `Interruptor` de `Campo`; esses arquivos continuam existindo até a Task 10.)
...
### Task 10: Subtelas do perfil — metas, dados, refeições
...
- **Cobertura da spec (Telas):** Login → T3; Início (saldo, `kcal a mais`, macros, lista D8, `ver calendário`, `?data`, excluir/trocar) → T4; Água (presets D6, sem teto D7, animação, só hoje) → T1+T5; Calendário (grade, hoje 700, painel com/sem dados, `?mes`, só leitura em PerfilPublico) → T6; Detalhe (D12, `PATCH alimentos`, remover último com confirmação, reaproveitado na confirmação) → T1+T7; Confirmação (folha, chips, `+ adicionar`, total, `C · P · G`) → T8; Perfil (avatar, METAS, REGISTRO, IDENTIDADE, APARÊNCIA D4, CONTA só `sair`) → T9; subtelas → T10; limpeza → T11.
- **Desvios anotados:** `adicionar refeição` no calendário navega para `/` em vez de abrir o menu (T6, falta `abrirMenu` em `useCaptura`); rota do detalhe usa `?data=` para achar o registro (não há `GET /api/registros/:id`); `ver relatório`/link `relatório` dependem do plano C.
